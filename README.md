...
***This is a draft of the protocol and implementation. Execpt issues (and report them) !***
...
server.py is the main flask file, providing different call to the framework, generated from actions defined in actions.py. Basic configuration is available at the begining of the file. Otherwhise, this file **should not be edited** !
...
It's possible to use various additionnal decorators to specify how the meta about the action is generated:

* `@only_logged_user()`: Make action available only for logged users
* `@only_member_user()`: Make action available only for memebers users
...
Again, the server.py file take care of responsing to /meta/, /template/ and /action/ call. The fonction  in actions.py will be called when needed. The request is passed as the first parameters to actions.
...
A _ebuio_baseUrl_ value is provied, with the root url to use for all url generation (others actions, medias, etc..)
...
If you use our Flask server, thoses method are automaticaly implemented !
...
* only_admin_user : Boolean. Optional, default to False.  True if the user must be an admin of the project on EBUio to call the action.
...
* user_info : List of string. Optional, default to []. List of user properties EBUio add to each request. Example properties, the full list isn't defined yet: username, email, first_name, last_name.
